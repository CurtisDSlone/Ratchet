...
func (d *Dispatcher) Help() string {
	...
	sb.WriteString("  /runs [n]                list recent chain runs (the audit/observability log)\n")
	sb.WriteString("  /rollback [id|latest]    restore the workspace to a run's pre-state (no id = preview latest)\n")
	sb.WriteString("  /snapshot                save a manual restore point of the active workspace\n")
